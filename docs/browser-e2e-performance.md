...
`.github/workflows/browser-e2e.yml` runs a pinned Chromium/Playwright production matrix on a daily schedule and on manual dispatch. The runner installs Playwright ephemerally after `npm ci`; it does not add the browser runner to the shipped application dependency graph.

The always-on anonymous matrix verifies:
...
This complements the HTTP production smoke: the HTTP smoke validates redirects, cache headers, manifest/icons, and service-worker source invariants; the browser E2E validates rendered browser behavior.

## Optional authenticated browser evidence

The runner recognizes dedicated GitHub Actions secrets when they are configured:
...
The workflow never prints the secret values. It does not create guest access, use a service-role key, or weaken RLS. Absence of optional test credentials is reported as a skip rather than a fabricated pass for authenticated coverage.
