...
async function signOut(page, label) {
  ...
  await page.getByRole('heading', { name: 'Your Pachimanga account' }).waitFor({ timeout: 20_000 });
...
async function runAuthenticatedMatrix(browser) {
  ...
  await first.page.getByRole('heading', { name: 'Your Pachimanga account' }).waitFor({ timeout: 20_000 });
  await first.page.getByRole('heading', { name: 'Personalize your account' }).waitFor({ timeout: 20_000 });
  await assertNoHorizontalOverflow(first.page, 'authenticated /account');
  await assertInteractiveLabels(first.page, 'authenticated /account');
  await assertKeyboardFocusVisible(first.page, 'authenticated /account');
