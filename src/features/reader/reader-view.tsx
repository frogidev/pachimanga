...
import {
  effectiveSpeed,
  nextMultiplier,
} from "@/features/reader/auto-scroll";
...
export function ReaderView({
  ...
  const speed = useMemo(
    () => effectiveSpeed(settings.baseSpeedPxPerSecond, settings.autoScrollMultiplier),
    [settings],
  );
  ...
  useEffect(() => {
    ...
    const onScroll = () => {
      ...
      scrollFrame.current = requestAnimationFrame(() => {
        ...
        if (Number.isFinite(index) && index !== state.currentPageIndex) {
          dispatch({ type: "page", index });
        }
  ...
  useEffect(() => {
    ...
    const preloads = upcoming.map((page) => {
  ...
  useEffect(() => {
    const onWheel = () => {
      if (state.autoScrollPlaying) pause();
    };
  ...
  const toggleControls = () => {
    dispatch({ type: state.controlsVisible ? "hide-controls" : "show-controls" });
  };
  ...
  return (
    <div
      ...
      onMouseMove={() => dispatch({ type: "show-controls" })}
    >
      <div className="mx-auto flex min-h-dvh w-full max-w-[1200px] flex-col items-center bg-zinc-950">
        {pages.map((page, index) => (
          <div
            ...
            className={`relative flex w-full justify-center overflow-hidden bg-zinc-900 ${index > 0 ? "border-t border-black" : ""}`}
            style={{ aspectRatio: page.width && page.height ? `${page.width} / ${page.height}` : "2 / 3" }}
          >
            <Image
              src={page.imageUrl}
              ...
              unoptimized
              className={
                settings.fitMode === "screen"
                  ? "h-auto max-h-[100svh] w-auto max-w-full object-contain"
      ...
      <div
        className={`fixed inset-x-0 top-0 z-50 transition duration-200 ${state.controlsVisible ? "translate-y-0 opacity-100" : "-translate-y-full opacity-0"}`}
      ...
      <div
        className={`fixed inset-x-0 bottom-0 z-50 transition duration-200 ${state.controlsVisible ? "translate-y-0 opacity-100" : "translate-y-full opacity-0"}`}
        onClick={(event) => event.stopPropagation()}
      >
          ...
          <div className="rounded-3xl border border-white/10 bg-black/85 p-3 shadow-2xl shadow-black/50 backdrop-blur-xl sm:p-4">
            <div className="flex items-center gap-3">
              <button
                ...
                className="grid size-12 shrink-0 place-items-center rounded-2xl bg-emerald-400 text-base font-black text-zinc-950 hover:bg-emerald-300"
                aria-label={state.autoScrollPlaying ? "Pause auto-scroll" : "Play auto-scroll"}
              >
                {state.autoScrollPlaying ? "Ⅱ" : "▶"}
              </button>
              <div className="min-w-0 flex-1">
                <div className="mb-1.5 flex items-center justify-between text-xs">
                  <span className="font-medium text-zinc-200">Auto-scroll</span>
                  <span className="tabular-nums text-zinc-500">{settings.autoScrollMultiplier.toFixed(settings.autoScrollMultiplier % 1 ? 2 : 0)}× · {Math.round(speed)} px/s</span>
                </div>
                <input
                  type="range"
                  min="0.1"
                  max="5"
                  step="0.05"
                  value={settings.autoScrollMultiplier}
                  onChange={(event) => updateSettings({ autoScrollMultiplier: Number(event.target.value) })}
                  className="w-full accent-emerald-400"
                  aria-label="Auto-scroll speed"
                />
              </div>
              <div className="flex shrink-0 gap-1">
                <button type="button" onClick={() => updateSettings({ autoScrollMultiplier: nextMultiplier(settings.autoScrollMultiplier, -1) })} className="grid size-9 place-items-center rounded-xl bg-white/8 text-lg hover:bg-white/12" aria-label="Decrease auto-scroll speed">−</button>
                <button type="button" onClick={() => updateSettings({ autoScrollMultiplier: nextMultiplier(settings.autoScrollMultiplier, 1) })} className="grid size-9 place-items-center rounded-xl bg-white/8 text-lg hover:bg-white/12" aria-label="Increase auto-scroll speed">+</button>
              </div>
            </div>
            ...
            <div className="mt-3 flex items-center justify-between gap-2 border-t border-white/8 pt-3 text-xs">
              ...
              <button
                type="button"
                onClick={() => updateSettings({ fitMode: settings.fitMode === "width" ? "screen" : "width" })}
                className="rounded-xl bg-white/8 px-3 py-2 text-zinc-300 hover:bg-white/12"
              >
                Fit {settings.fitMode === "width" ? "width" : "screen"}
              </button>
              <button
                type="button"
