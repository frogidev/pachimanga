...
import { isAppleMobileDevice, isInstalledPwa } from "@/lib/pwa/display-mode";
...
function initialAppleMobile() {
  return isAppleMobileDevice();
}

function initialInstalled() {
  return isInstalledPwa();
}

export function PwaInstall() {
  const [appleMobile] = useState(initialAppleMobile);
  ...
  return (
    <div className="app-page max-w-4xl">
      <div className="rounded-3xl border border-pink-300/15 bg-gradient-to-br from-pink-300/[.08] via-[#111019] to-[#0d0c12] p-6 sm:p-8">
        ...
        <h1 className="mt-2 text-3xl font-bold tracking-[-.04em] text-white sm:text-4xl">{installed ? "Pachimanga is installed" : "Install Pachimanga as a PWA"}</h1>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">
          {installed
            ? "This standalone app uses the same private account, synchronized library and reader as the browser version. Updates are offered in-app when a new service worker is ready."
            : "The PWA is the free distribution path for iPhone, iPad, Android and desktop. It uses the same account, library, reader and private WeebCentral relay as the web app."}
        </p>

        {installed ? (
          <div className="mt-6 rounded-2xl border border-emerald-300/20 bg-emerald-300/[.06] px-4 py-3 text-sm text-emerald-200">
            Standalone mode detected. No installation steps are needed on this device.
          </div>
        ) : promptEvent ? (
        ...
        <div className="mt-5 grid gap-4 md:grid-cols-2">
          ...
          <section className="surface-card p-5 sm:p-6">
            ...
            <div className="mt-5 rounded-xl border border-white/[.07] bg-black/15 px-4 py-3 text-xs leading-5 text-zinc-500">
              Native Android, Windows, Linux and macOS builds remain optional. The PWA is the zero-cost path and receives UI updates through the in-app update prompt.
            </div>
          </section>
        </div>
      ) : (
        <section className="surface-card mt-5 p-5 sm:p-6">
          <p className="pixel-kicker text-[9px] text-sky-400">Update behavior</p>
          <h2 className="mt-2 text-lg font-semibold text-zinc-100">Updates do not interrupt reading</h2>
          <p className="mt-2 text-sm leading-6 text-zinc-500">A newly downloaded app version waits in the background. Pachimanga shows an Update & reload banner, and the current version remains active until you accept it.</p>
        </section>
      )}
